...
int HashAusgabe(){
	...
	if((HASH->SR &2) ==2){
				for (int i = 0; i<=4; i++){
				sprintf(ascii, "%x",HASH->HR[i] );
				if (i<3) LCD_WriteString(1+ i *12*9, 41 +absatz, 0b0110001101111101,0b1111100000000000, ascii);
				else LCD_WriteString(1+ (i-3) *12*9, 61+ absatz, 0b0110001101111101,0b1111100000000000, ascii);
				}
				absatz = 80;
				HASH->SR &=~(1<<1);
				}
...
int HashChar(char string[]){
    ...
    // HASH neu initialisieren // Bin mir nicht sicher ob ich das brauche, da in HashAusgabe Hash-> Sr null gesetzt wird
		HASH->CR &= ~(1 << 2); // INIT Bit setzen
    HASH->CR |= (1 << 2); // INIT Bit setzen

    // Daten in 32-bit-Wörtern schreiben
    uint32_t data;
    for (int i = 0; i < len; i += 4) {
        data = 0;
        for (int j = 0; j < 4; j++) {
            if (i + j < len) {
                data |= (uint8_t)string[i + j] << (8 * j);  // Byteweise einfügen
            }
        }
        HASH->DIN = data; // Daten einfüllen
    }
    HASH->STR |= (1 << 8); //DCAL Bit setzen
...
int Hash_Init(){
	RCC->AHB2ENR |= 1<<5;
	HASH->CR &= ~(0b111111<<2);//Bits 2 bis 7 Null inklusive Mode und Algo
	HASH->CR |= (1<<5);//Datatype einstellen
	HASH->CR |= (1<<2);// Init bit setzen
	return 0;
}

int main(void){	
	char input[] = "Hallo Welt 2025!";
