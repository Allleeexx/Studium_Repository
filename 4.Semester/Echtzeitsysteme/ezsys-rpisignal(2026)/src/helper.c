 ...
 */
FILE* setup_gnuplot();
void plot_to_gnuplot(measurement_t* m, size_t num, FILE* gp, uint64_t half_period_ns);
...
gpio_handle_t* init_gpio(int gpio_pin, const char* gpio_chip) {
    ...
    handle->offset = (unsigned int)gpio_pin;

    struct gpiod_line_settings* settings = gpiod_line_settings_new();
    if (!settings) {
        perror("Fehler beim Erstellen der GPIO-Leitungseinstellungen");
    ...
    if (!line_cfg || gpiod_line_config_add_line_settings(line_cfg, &handle->offset, 1, settings) < 0) {
    ...
    if (req_cfg) {
        gpiod_request_config_set_consumer(req_cfg, "RPiSignal");
    }

    handle->request = gpiod_chip_request_lines(handle->chip, req_cfg, line_cfg);
    ...
    if (!handle->request) {
        perror("Fehler beim Anfordern der GPIO-Leitung");
        gpiod_chip_close(handle->chip);
        free(handle);
        return NULL;
    }
...
void* func_data_handler(void* args) {
    ...
    while (!param->killswitch) {
        ...
        if (param->doPlot) {
            plot_to_gnuplot(all_measurements, all_count, gp, param->half_period_ns);
        }
...
void plot_to_gnuplot(measurement_t* m, size_t num, FILE* gp, uint64_t half_period_ns) {
    if (num > 0) {
        ...
        for (size_t i = start_index; i < num; i++) {
            ...
            int64_t jitter = abs(diff - (int64_t)half_period_ns);
