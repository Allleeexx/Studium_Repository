...
In 2026 arbeiten wir direkt auf dem Raspberry Pi 5 (Ubuntu). Der User ist ezsys, das Passwort ist 'sose2026'.

### 1.3 Laden des Code-Templates

Das für dieses Praktikum benötigte Code-Template wird als ZIP via Moodle bereitgestellt. 
...
Gehen Sie in das Verzeichnis build und rufen Sie make auf.


## 2 Tests auf Standard-Ubuntu 26.04
...
Für die Ansteuerung der GPIO-Pins wird die Bibliothek `libgpiod` (Version 3) verwendet. Die Initialisierung des Pins erfolgt bereits durch die bestehende Initialisierungsroutine. Ihre Aufgabe ist lediglich, den Pin auf 0 oder 1 zu setzen. Hierfür verwenden Sie folgenden Funktionsaufruf:

```c
gpiod_line_request_set_value(gpio_request, gpio_offset, value);
```

- `gpio_request` erhalten Sie über `param->gpio->request`
- `gpio_offset` erhalten Sie über `param->gpio->offset`
- `value` ist der gewünschte Ausgabewert (`GPIOD_LINE_VALUE_INACTIVE` oder `GPIOD_LINE_VALUE_ACTIVE`)
...
2. Testen Sie Ihre Implementierung mit einer gewünschten Frequenz und überprüfen Sie das erzeugte Signal mit einem Oszilloskop:
...
Bauen Sie das Programm mittels make und führen Sie es aus:

Führen Sie die Anwendung aus und exportieren Sie die Zeitdifferenzen in eine Datei:
